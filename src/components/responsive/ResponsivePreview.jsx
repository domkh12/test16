...
import { createContact, updateContact } from "../../redux/feature/websitetemplate/ContactSlice";
import AOS from "aos";
...
const ResponsivePreview = ({ children, device, setDevice }) => {
  ...
  const saveData = () => {
    contacts.forEach((contact) => {
      if (contact.id) {
        // If the contact has an ID, it exists, so update it
        dispatch(updateContact(contact));
      } else {
        // If there's no ID, it doesn't exist yet, so create it
